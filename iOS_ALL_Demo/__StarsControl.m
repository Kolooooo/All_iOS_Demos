...
#import "__StarsControl.h"
#import "__StarButton.h"

@interface __StarsControl()

@property (nonatomic, strong) __StarButton *currentStar;
...
- (void)setupView {
    for (NSInteger index = 0; index < self.numberOfStars; index++) {
        __StarButton *starButton = [__StarButton.alloc initWithSize:self.starSize];
...
- (void)layoutSubviews {
    ...
    for (NSInteger index = 0; index < self.numberOfStars; index ++) {
        __StarButton *starButton =  [self starForTag:index];
...
- (__StarButton *)starForPoint:(CGPoint)point {
    for (NSInteger i = 0; i < self.numberOfStars; i++) {
        __StarButton *starButton = [self starForTag:i];
...
- (__StarButton *)starForTag:(NSInteger)tag {
...
- (BOOL)beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    __StarButton *pressedStar = [self starForPoint:point];
    if (pressedStar) {
