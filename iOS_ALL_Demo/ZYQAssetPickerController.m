...
/// 相册图片的尺寸大小
#define kThumbnailLength  ([UIScreen mainScreen].bounds.size.width-10.0)*0.333 //#define kThumbnailLength    78.0f 原来参数
...
#pragma mark - ZYQTapAssetView 选中相册的item
...
#pragma mark - ZYQAssetView 相册图片的item
...
#pragma mark - ZYQTapAssetViewDelegate
-(BOOL)shouldTap{
    ...
    if (self.contentView.subviews.count<assets.count) {
        for (int i=0; i<assets.count; i++) {
            if (i > ((NSInteger)self.contentView.subviews.count - 1)) {
                ...
                ZYQAssetView *assetView = [[ZYQAssetView alloc] init];
                CGFloat x = assetViewX + (kThumbnailSize.width+minimumInteritemSpacing)*i;
                CGRect frame =  CGRectMake(x, minimumLineSpacing-1, kThumbnailSize.width, kThumbnailSize.height);
...
#pragma mark - ZYQAssetViewDelegate
...
#pragma mark - __ZYQAssetViewController
...
#pragma mark - ZYQAssetViewCell 相册添加cell
